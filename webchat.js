...
const WebchatPreview = () => {
  ...
  }, [isMounted, backgroundUrls.length]);
  ...
  // Enhanced button with animation - Fixed to avoid window reference during SSR
  const AnimatedButton = ({ text, style }) => {
    // Calculate transform based on cursor position only on client side
    const gradientTransform = isMounted ? {
      transform: `translate(
        ${(cursorPosition.x / (typeof window !== 'undefined' ? window.innerWidth : 1)) * 100 - 50}%, 
        ${(cursorPosition.y / (typeof window !== 'undefined' ? window.innerHeight : 1)) * 100 - 50}%
      )`
    } : {};
